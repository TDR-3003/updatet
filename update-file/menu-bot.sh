...
RED="\033[0;31m" 
COLOR1="$(cat /etc/ryzvpn/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
COLBG1="$(cat /etc/ryzvpn/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"                    
###########- END COLOR CODE -##########
...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/ryz-code/izin-vps/master/bot > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/ryz-code/izin-vps/master/bot | grep $MYIP | awk '{print $2}')
Isadmin=$(curl -sS https://raw.githubusercontent.com/ryz-code/izin-vps/master/bot | grep $MYIP | awk '{print $5}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/ryz-code/izin-vps/master/bot | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
    res="Izin Ditolak!"
...
Exp=$(curl -sS https://raw.githubusercontent.com/ryz-code/izin-vps/master/bot | grep $MYIP | awk '{print $3}')
fi
